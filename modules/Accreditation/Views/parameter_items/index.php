 <form class="" action="<?=base_url() . 'msi'?>" method="get">
   <div class="row">
       <div class="col-md-3">
         <select name="academic_program_id" class="form-control <?= $errors['academic_program_id'] ? 'is-invalid':'is-valid' ?>" required>
       ...
       <div class="col-md-3">
         <select name="accreditation_level_id" class="form-control <?= $errors['accreditation_level_id'] ? 'is-invalid':'is-valid' ?>" required>
       ...
       <div class="col-md-3">
         <select name="area_id" class="form-control <?= $errors['area_id'] ? 'is-invalid':'is-valid' ?>" required>
       ...
       <ul class="nav nav-tabs">
         <?php if(!empty($item_parameters)): ?>
           <?php foreach ($item_parameters as $item_parameter): ?>
             <li class="nav-item">
               <a class="ml-2 btn btn-outline-secondary" onClick= "displayMSI(<?= $item_parameter['template_parameter_id'] ?>, <?= $item_parameter['accreditation_template_id'] ?>)"  href="#"><?= strtoupper($item_parameter['parameter_code']) ?><br><?= ucwords($item_parameter['title']) ?></a>
             </li>
           <?php endforeach; ?>
           <?php else: ?>
             <?php if (isset($rec)): ?>
